...
alias ttmux="TERM=xterm-256color tmux new-session -A -s Default"
...
function nnn --wraps nnn --description "Open nnn with preview enabled"
    set -lx NNN_PLUG "p:preview-tui"
    set -lx NNN_PREVIEWIMGPROG "chafa --format=symbols"
    set -lx NNN_SPLIT v
    set -lx NNN_SPLITSIZE 40
    set -lx EDITOR emacs
    set -lx VISUAL emacs

    command nnn -a -eE -P p -s default -S $argv
end
...
function eg --description "Open Magit status in the current directory"
    set -lx EG_MAGIT_DIR (pwd)
    command emacs --chdir "$EG_MAGIT_DIR" --eval '(progn (setq magit-display-buffer-function (quote magit-display-buffer-fullframe-status-v1)) (magit-status (getenv "EG_MAGIT_DIR")))'
end
