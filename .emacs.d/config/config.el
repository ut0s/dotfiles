...
(use-package emacs
  ...
  (setq
    ...
    ;; todo
    org-todo-keywords
    '((sequence "TODO(t!)" "HANDOVER(h!)" "URGENT(u!)" "WAIT(w!)" "|" "DONE(d!)" "CANCELED(c!)"))
    ;; tag/property
...
(use-package elfeed
  :ensure t
  :commands (elfeed-search-open-in-w3m-url)
  :init
  (defun elfeed-search-open-in-w3m-url (&optional use-generic-p)
    (interactive "P")
  ...
  :bind (
          ...
          ("e" . #'elfeed-search-open-in-w3m-url)
...
(use-package git-complete
  :ensure nil
  :load-path "site-lisp"
  :bind ("C-c C-c" . git-complete)
  :custom (git-complete-repeat-completion nil git-complete-enable-autopair t))

;; (use-package git-complete-company
;;   :ensure nil
;;   :load-path "site-lisp"
;;   :after (git-complete)
;;   :
;;   (add-to-list 'company-backends 'git-complete-company-omni-backend)
;;   (add-to-list 'company-backends 'git-complete-company-whole-line-backend)
;; )
