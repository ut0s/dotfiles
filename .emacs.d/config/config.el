...
;; Background colors.
(set-background-color "black") ;; Default background color.
(set-face-background 'default "black") ;; Default background color.
(set-face-foreground 'default "white") ;; Default foreground color.
(setq frame-background-mode 'dark) ;; Use dark-background face variants.
(mapc #'frame-set-background-mode (frame-list))

;; hilit19 setting.
(setq hilit-background-mode 'dark) ;; Use dark-background highlights.
...
(use-package markdown-mode
  ...
  :mode ("\\.md\\'" . gfm-mode)
  :init
  (defun my/apply-gfm-mode-to-markdown-buffers ()
    "Use `gfm-mode' for Markdown buffers restored before auto-mode setup."
    (dolist (buffer (buffer-list))
      (with-current-buffer buffer
        (when (and buffer-file-name
          ...
          (gfm-mode)))))
  (add-hook 'desktop-after-read-hook #'my/apply-gfm-mode-to-markdown-buffers)
  :config
